...
export abstract class ElderSqlConnection {
	abstract select(
		text: string,
	): ElderSqlSelecter<Record<string, any>, Record<string, any>>;
	abstract select<P, R>(
		builder: (con: ElderSqlConnection) => ElderSqlSelecter<P, R>,
	): ElderSqlSelecter<P, R>;

	abstract execute(text: string): ElderSqlExecuter<Record<string, any>, any>;
	abstract execute<P, R>(
		builder: (con: ElderSqlConnection) => ElderSqlExecuter<P, R>,
	): ElderSqlExecuter<P, R>;
}
...
export abstract class ElderSqlDriver {
	...
	abstract transaction<T = void>(
		fn: (con: ElderSqlConnection) => Promise<T>,
	): Promise<T>;
	abstract transaction<T = void>(
		options: ElderSqlTransactionOptions,
		fn: (con: ElderSqlConnection) => Promise<T>,
	): Promise<T>;
